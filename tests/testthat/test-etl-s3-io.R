...
test_that("aws_cli_args injects --endpoint-url when set", {
  old <- Sys.getenv("S3_ENDPOINT_URL")
  on.exit(Sys.setenv(S3_ENDPOINT_URL = old), add = TRUE)
  Sys.setenv(S3_ENDPOINT_URL = "https://example.test")
  args <- aws_cli_args(c("s3api", "list-objects-v2"))
  expect_equal(args[1:2], c("--endpoint-url", "https://example.test"))
})

test_that("aws_cli_args is pass-through when endpoint env var is empty", {
  old <- Sys.getenv("S3_ENDPOINT_URL")
  on.exit(Sys.setenv(S3_ENDPOINT_URL = old), add = TRUE)
  Sys.setenv(S3_ENDPOINT_URL = "")
  args <- aws_cli_args(c("s3", "cp", "src", "dst"))
  expect_equal(args, c("s3", "cp", "src", "dst"))
})

test_that("aws_cli_env contains expected access key vars", {
  old_id <- Sys.getenv("AWS_ACCESS_KEY_ID")
  old_secret <- Sys.getenv("AWS_SECRET_ACCESS_KEY")
  on.exit({
    Sys.setenv(AWS_ACCESS_KEY_ID = old_id)
    Sys.setenv(AWS_SECRET_ACCESS_KEY = old_secret)
  }, add = TRUE)
  Sys.setenv(AWS_ACCESS_KEY_ID = "test-id",
             AWS_SECRET_ACCESS_KEY = "test-secret")
  env <- aws_cli_env()
  expect_true(any(grepl("AWS_ACCESS_KEY_ID=test-id", env)))
  expect_true(any(grepl("AWS_SECRET_ACCESS_KEY=test-secret", env)))
})
