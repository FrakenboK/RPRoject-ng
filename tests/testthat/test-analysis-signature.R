...
test_that("condition_to_sql expands `1 of *` to OR group", {
  sel_sql <- list(
    selection_a = "a = 1",
    selection_b = "b = 1",
    selection_c = "c = 1"
  )
  sql <- condition_to_sql("1 of selection_*", sel_sql)
  expect_match(sql, "a = 1")
  expect_match(sql, "b = 1")
  expect_match(sql, "c = 1")
  expect_match(sql, "OR")
})

test_that("condition_to_sql expands `all of *` to AND group", {
  sel_sql <- list(
    selection_a = "a = 1",
    selection_b = "b = 1"
  )
  sql <- condition_to_sql("all of selection_*", sel_sql)
  expect_match(sql, "AND")
})
...
test_that("build_sigma_sql compiles C2 listener rule with gte modifiers", {
  rule <- list(
    id = "test-c2",
    detection = list(
      selection_ports = list(dst_port = c(4444, 5555)),
      selection_duration = list("duration_sec|gte" = 30),
      selection_bytes = list("bytes_total|gte" = 1000),
      condition = "selection_ports and selection_duration and selection_bytes"
    )
  )
  sql <- build_sigma_sql(rule)
  expect_match(sql, "dst_port = 4444")
  expect_match(sql, "dst_port = 5555")
  expect_match(sql, "duration_sec >= 30")
  expect_match(sql, "bytes_total >= 1000")
})

test_that("build_sigma_sql rejects rule without selections or condition", {
  expect_error(
    build_sigma_sql(list(detection = list(condition = "selection_a"))),
    "no selections"
  )
  expect_error(
    build_sigma_sql(list(detection = list(selection_a = list(x = 1)))),
    "no detection condition"
  )
})

test_that("load_sigma_rules returns empty list for missing dir", {
  result <- load_sigma_rules(tempfile(pattern = "no-such-dir-"))
