...
write_unsw_csv_with_header <- function() {
  header <- paste(UNSW_NB15_COLUMNS, collapse = ",")
  row <- paste(rep("0", length(UNSW_NB15_COLUMNS) - 5), collapse = ",")
  full_row <- paste("1.1.1.1,80,2.2.2.2,443,tcp", row, "Normal,0", sep = ",")
  full_row <- paste(c("1.1.1.1", "80", "2.2.2.2", "443", "tcp", "FIN", "1.0",
                      "100", "200", "64", "64",
                      rep("0", length(UNSW_NB15_COLUMNS) - 13),
                      "Normal", "0"), collapse = ",")
  path <- tempfile(fileext = ".csv")
  writeLines(c(header, full_row), path)
  path
}
...
test_that("parse_csv_file auto-detects headered UNSW-NB15", {
  path <- write_unsw_csv_with_header()
  on.exit(unlink(path), add = TRUE)

  src <- make_source_object("raw/unsw/headered.csv", "unsw-nb15", "csv")
  df <- parse_csv_file(path, src, "run-2")
  expect_equal(nrow(df), 1)
  expect_equal(df$src_ip, "1.1.1.1")
  expect_equal(df$dst_ip, "2.2.2.2")
})
